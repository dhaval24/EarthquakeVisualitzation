...
public class ParseFeed {
	...
	public static Map<String, Float> loadGDPFromCSV(PApplet p, String fileName){

		Map<String, Float> gdpMap = new HashMap<>();
		String rows[] = p.loadStrings(fileName);
		for(String row: rows){
			String columns[] = row.split(",");
			for(int i = columns.length -1; i > 4; i--){
				if(columns[i].length() > 2) {
					gdpMap.put(columns[1].substring(1, columns[1].length()-1), Float.parseFloat(columns[i].substring(1,columns[i].length()-1)));
					break;
				}
			}
		}
